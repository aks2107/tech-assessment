...
def check_directories():
    ...
    If the input files or data directory is not present then the program terminates alerting the user of the missing item.
    """
...
def read_member_info():
    """
    This function reads the information from memberInfo.csv into a dictionary for quick lookups.
    ...
    with open(MEMBERINFO_FILE, 'r') as file:
        reader = csv.DictReader(file) # First line in file gets treated as keys which are the columns and everything following gets treated as values for the keys
        for row in reader: 
            ...
            full_name = f"{first_name} {last_name}".strip() # Combine first and last name into one string and remove white space
            members[member_id] = full_name # Populate dictionary with key as member id and value as full name
    return members

def read_member_paid_info():
    """
    This function reads the information from memberPaidInfo.csv file into a dictionary for quick lookups.
    ...
    with open(MEMBERPAIDINFO_FILE, 'r') as file:
        ...
        for row in reader:
            t_id = row['memberId'].strip() # Get 'memberID' value for that specific row
            t_name = row['fullName'].strip() # Get 'fullName' value for that specific row
            # Try and except block used to handle price data that may not be numerical
            try:
                t_price = int(row['paidAmount']) # Convert price to int
            except ValueError: # If the price value cannot be converted to int
                continue # Skip row and move to next
...
def main():
    ...
    3. Goes through list of transactions and checks if a valid member exists for that transaction and gets rid of conflicting names
    4. Gets total paid amount
    5. Gets highest paying member
    6. Creates output file with the clean entries
    7. Prints out some insights about the clean entires
    """
    ...
    total_paid = 0.0
    clean_entries = []
    highest_payer = {'fullName': None, 'paidAmount': 0, 'memberID': None}
    for row in transactions:
        payment_id = row['ID']
        payment_name = row['fullName']
        payment_amount = row['paidAmount']
        if payment_id not in members: # Check if ID found in transactions exists in member information list
            continue # Skip ID if not associated with member
        if (members[payment_id].upper() != payment_name.upper()) and (payment_name != '') and (members[payment_id] != ''): # Check for conflicting names
            continue # Skip name if it is conflicting with names in member list
        if(payment_name == ''): # Check if name in transaction list is empty
            payment_name = members[payment_id] # Fill in the name of member for the transaction
        total_paid += payment_amount # Add to total paid count
        if payment_amount > highest_payer['paidAmount']: # Check if transaction amount if greater than the current highest paid amount
    ...
    with open(OUTPUT_FILE, 'w', newline='') as file:
        columns = ['memberID', 'fullName', 'paidAmount'] # Columns for output file
